...
function match(nodeArrays, fileContents) {
  const m = new Match(nodeArrays);
  m.populateLines(fileContents);
  m.instances =
    _.uniqBy(v => JSON.stringify([v.filename, v.start.line, v.end.line]), m.instances);
